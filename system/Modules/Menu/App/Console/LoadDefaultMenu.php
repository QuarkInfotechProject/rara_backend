...
class LoadDefaultMenu extends Command
{
    ...
    public function insertMenuItems($items, $parentId = 0) {
        foreach ($items as $item) {
            ...
            // <-- Add your permission syncing code here:
            if (!empty($item['permission_name'])) {
                $permission = \Spatie\Permission\Models\Permission::where('name', $item['permission_name'])->first();

                if ($permission) {
                    DB::table('permission_menu')->updateOrInsert([
                        'permission_id' => $permission->id,
                        'menu_id' => $itemId,
                        'group_id' => 1, // <-- replace 1 with the correct group ID
                    ]);
                }
            }
